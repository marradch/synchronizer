...
class VKAuthService
{
    ...
    public function __construct()
    {
		...
		//$redirect_uri = $protocol.$_SERVER['HTTP_HOST'].'/authRedirect'; 
		$this->redirectURI = $protocol.'e967cf16.ngrok.io'.'/authRedirect';
	...
	{
		$tokenItem = $this->token->first();
		if($tokenItem){
			
			$vk = new VKApiClient();
		
			try{
				$response = $vk->users()->get($tokenItem->token);
